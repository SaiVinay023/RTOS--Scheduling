...
//proper syscall numbers should be used
#ifdef __x86_64__
...
//characteristic function of the thread, for timing and synchronization
...
//Suspend the task until beginning of the next period
//use nanosleep
...
//compute U
	...
	//declare variables to read current time
	struct timeval ora;
	struct timezone zona;
	gettimeofday(&ora, &zona);
	...
	// period, but the end of the first period and beginning of the next one. 
  	for (i = 0; i < NTASKS; i++)
    	{
		// first to encode current time in nanoseconds and add the period 
		//long int next_arrival_nanoseconds =( time_1.tv_sec*1000000000 + time_1.tv_sec + period[i];
	...
	sleep(10);
// used for joining threads (pthread_join)
